...
//////////////
  ...
  octomap_msgs::Octomap octree_ros;
    octree_ros.header.frame_id = "guidance";
   octree_ros.header.stamp    = ros::Time::now();
 // octomap_msgs::binaryMapToMsg(tree, octree_ros);
  octree_ros.resolution=0.05;
  pub_octomap.publish(octree_ros);
...
//pcl::StatisticalOutlierRemoval
// Container for original & filtered data
  pcl::PCLPointCloud2* cloud2 = new pcl::PCLPointXYZ; 
  pcl::PCLPointCloud2ConstPtr cloudPtr(cloud2);
  pcl::PCLPointCloud2 cloud_filtered;
// Convert to PCL data type
  pcl_conversions::toPCL(output, *cloud2);
// Create the filtering object
  pcl::RadiusOutlierRemoval<pcl::PCLPointXYZ> outrem;
...
int main(int argc, char** argv)
{
  ...
  pub_octomap = my_node.advertise<octomap_msgs::Octomap> ("octomap_output", 1);
          ros::Rate r(0.5);	
    while (ros::ok())
       {
