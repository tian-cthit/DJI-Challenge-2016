...
Mat				depthshow1(HEIGHT, WIDTH, CV_8UC1);
...
void crimminsImage(Mat image,int width,int height, int iterations)
{
	//image = cvCreateMat(240,320,CV_8UC1);
        Mat image2 = image.clone();
       // image2 = cvCreateMat(240,320,CV_8UC1);

	for(int its=0;its<iterations;its++){
      for(int j=0;j<height;j++){
	 for(int i=0;i<width;i++){
	  if(i-1>=0 && image.at<uchar>(i-1,j) > (image.at<uchar>(i,j)+1)){
	    image2.at<uchar>(i,j) = image.at<uchar>(i,j) +1;
	  }
	  else image2.at<uchar>(i,j) = image.at<uchar>(i,j);
	}
      }
      for(int j=0;j<height;j++){
	for(int i=0;i<width;i++){
	  if(i-1>=0 && i+1<width
	     && image2.at<uchar>(i-1,j) > image2.at<uchar>(i,j) && image2.at<uchar>(i+1,j) >= image2.at<uchar>(i,j)){
	    image.at<uchar>(i,j) = image2.at<uchar>(i,j) +1;
	  }
	  else image.at<uchar>(i,j) = image2.at<uchar>(i,j);
	}
      }
      for(int j=0;j<height;j++){
	for(int i=0;i<width;i++){
	  if(i+1<width && i-1>=0
	     && image.at<uchar>(i-1,j) >= image.at<uchar>(i,j) && image.at<uchar>(i+1,j) > image.at<uchar>(i,j)){
	    image2.at<uchar>(i,j) = image.at<uchar>(i,j) +1;
	  }
	  else image2.at<uchar>(i,j) = image.at<uchar>(i,j);
	}
      ...
      for(int j=0;j<height;j++){
	for(int i=0;i<width;i++){
	  if(i+1<width && image2.at<uchar>(i+1,j) > (image2.at<uchar>(i,j)+1)){
	    image.at<uchar>(i,j) = image2.at<uchar>(i,j) +1;
	  }
	  else image.at<uchar>(i,j) = image2.at<uchar>(i,j);
	}
	...
	for(int i=0;i<width;i++){
      ...
      for(int j=0;j<height;j++){
	for(int i=0;i<width;i++){
	  if(j-1>=0 && image.at<uchar>(i,j-1) >(image.at<uchar>(i,j)+1)){
	    image2.at<uchar>(i,j) = image.at<uchar>(i,j) +1;
	  }
	  else image2.at<uchar>(i,j) = image.at<uchar>(i,j);
	}
	...
	for(int i=0;i<width;i++){
	  if(j-1>=0 && j+1<height
	     && image2.at<uchar>(i,j-1) > image2.at<uchar>(i,j) && image2.at<uchar>(i,j+1) >= image2.at<uchar>(i,j)){
	    image.at<uchar>(i,j) = image2.at<uchar>(i,j) +1;
	  }
	  else image.at<uchar>(i,j) = image2.at<uchar>(i,j);
	}
	...
	for(int i=0;i<width;i++){
	  if(j+1<height && j-1>=0
	     && image.at<uchar>(i,j-1) >= image.at<uchar>(i,j) && image.at<uchar>(i,j+1) > image.at<uchar>(i,j)){
	    image2.at<uchar>(i,j) = image.at<uchar>(i,j) +1;
	  }
	  else image2.at<uchar>(i,j) = image.at<uchar>(i,j);
	}
      ...
      for(int j=0;j<height;j++){
	for(int i=0;i<width;i++){
	  if(j+1<height && image2.at<uchar>(i,j+1) > (image2.at<uchar>(i,j)+1)){
	    image.at<uchar>(i,j) = image2.at<uchar>(i,j) +1;
	  }
	  else image.at<uchar>(i,j) = image2.at<uchar>(i,j);
	}
	...
	for(int i=0;i<width;i++){
	  if(j-1>=0 && j+1<height
	     && image2.at<uchar>(i,j-1) < image2.at<uchar>(i,j) && image2.at<uchar>(i,j+1) <= image.at<uchar>(i,j)){
	    image.at<uchar>(i,j) = image2.at<uchar>(i,j) -1;
	  }
	  else image.at<uchar>(i,j) = image2.at<uchar>(i,j);
	}
	...
	for(int i=0;i<width;i++){
	  if(i+1<width && j-1>=0
	     && image.at<uchar>(i+1,j-1) > (image.at<uchar>(i,j)+1)){
	    image2.at<uchar>(i,j) = image.at<uchar>(i,j)+1;
	  }
	  else image2.at<uchar>(i,j) = image.at<uchar>(i,j);
	}
	...
	for(int i=0;i<width;i++){
	  if(i-1>=0 && i+1<width && j-1>=0 && j+1<height
	     && image2.at<uchar>(i+1,j-1) > image2.at<uchar>(i,j)
	     && image2.at<uchar>(i-1,j+1) >= image2.at<uchar>(i,j)){
	    image.at<uchar>(i,j) = image2.at<uchar>(i,j)+1;
	  }
	  else image.at<uchar>(i,j) = image2.at<uchar>(i,j);
	...
	for(int i=0;i<width;i++){
	  if(i-1>=0 && i+1<width && j-1>=0 && j+1<height
	     && image.at<uchar>(i+1,j-1) >= image.at<uchar>(i,j)
	     && image.at<uchar>(i-1,j+1) > image.at<uchar>(i,j)){
	    image2.at<uchar>(i,j) = image.at<uchar>(i,j) +1;
	  }
	  else image2.at<uchar>(i,j) = image.at<uchar>(i,j);
	...
	for(int i=0;i<width;i++){
	  if(i-1>=0 && j+1<height
	     && image2.at<uchar>(i-1,j+1) > (image2.at<uchar>(i,j)+1)){
	    image.at<uchar>(i,j) = image2.at<uchar>(i,j) +1;
	  }
	else image.at<uchar>(i,j) = image2.at<uchar>(i,j);
	}
	...
	for(int i=0;i<width;i++){
	  if(i-1>=0 && j-1>=0
	    ...
	    image.at<uchar>(i,j) = (image2.at<uchar>(i,j) -1);
	      ...
	      for(int j=0;j<height;j++){
		 for(int i=0;i<width;i++){
			 depthshow1.at<uchar>(i,j) =  image.at<uchar>(i,j);
		}
	      }
  ...
  /* image data */
    ...
    {        
		...
		if ( data->m_depth_image[CAMERA_ID] ){
			...
			imshow("depth1", depth8);
			crimminsImage(depth8,240,320,k);
			imshow("depth", depthshow1);
			//publish depth image
