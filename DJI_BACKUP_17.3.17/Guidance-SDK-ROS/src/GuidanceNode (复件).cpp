...
int my_callback(int data_type, int data_len, char *content)
{
    ...
    {        
		...
		if ( data->m_depth_image[CAMERA_ID] ){
			...
			imshow("depth", depth8);
			//publish depth image
                            // Mat* depth_wqf=depth8;
                             //printf("depth=====%f",cvmGet(depth_wqf,120,160));
                //     printf("Depth at point (%d,%d) is %f meters!\n", HEIGHT/2+10, WIDTH/2,  float(g_depth.at<short>( HEIGHT/2,WIDTH/2))/128);
			...
			depth_image_pub.publish(depth_16.toImageMsg());
                      printf("Depth at point (%d,%d) is %f meters!\n", HEIGHT/2+10, WIDTH/2,  float( depth_16.image.at<short>( HEIGHT/2,WIDTH/2))/128);
