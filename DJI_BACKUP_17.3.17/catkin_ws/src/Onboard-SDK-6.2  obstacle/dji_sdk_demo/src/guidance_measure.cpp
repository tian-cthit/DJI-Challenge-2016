   ...
   //printf("Ford_150.linear.x000=%f\n",Ford_150.linear.x); 
       for(int i=0;i<10;i++)
		...
		while(ros::ok())
		{
			...
			//feedback_x=drone->local_position.x;
			//feedback_y=drone->local_position.y;

			float feedback_vx=drone->velocity.vx;
			float feedback_vy=drone->velocity.vy;
			printf("feedback_vx=%f\n",feedback_vx);
			printf("feedback_vy=%f\n",feedback_vy);
  
		        fprintf(fpwny," feedback_vx=%f\t  feedback_vy=%f\t \n",feedback_vx,feedback_vy);
			fflush(fpwny);
