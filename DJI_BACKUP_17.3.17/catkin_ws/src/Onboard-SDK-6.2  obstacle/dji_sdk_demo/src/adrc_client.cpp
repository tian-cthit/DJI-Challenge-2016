...
void PID_x_realize(float input1)	//PID algorithm
{
	...
	pid_x.v=3*(pid_x.kp*pid_x.error+pid_x.ki*pid_x.integral+pid_x.kd*(pid_x.error-pid_x.error_last));		//a_command
...
void PID_y_realize(float input2)	
{
	...
	pid_y.v=3*(pid_y.kp*pid_y.error+pid_y.ki*pid_y.integral+pid_y.kd*(pid_y.error-pid_y.error_last));		//a_command
...
int main(int argc, char **argv)
{
   ...
   //printf("Ford_150.linear.x000=%f\n",Ford_150.linear.x); 
       for(int i=0;i<10;i++)
		...
		float x_limit=12.0;			//a_command
		float y_limit=12.0;			//a_command
		...
		while(ros::ok())
		{
			...
			{	
				...
				/*m_input=sqrt(x_input*x_input+y_input*y_input);
				m_speed=sqrt(pid_x.v*pid_x.v+pid_y.v*pid_y.v);
				pid_h.v=(h_input/m_input)*m_speed;*/
			...
			drone->attitude_control( Flight::HorizontalLogic::HORIZONTAL_ATTITUDE_TILT_ANGULAR |		//command for a_x and a_y
                            Flight::VerticalLogic::VERTICAL_VELOCITY |
                            Flight::YawLogic::YAW_PALSTANCE |
                            Flight::HorizontalCoordinate::HORIZONTAL_GROUND |
                            Flight::SmoothMode::SMOOTH_ENABLE,
                            pid_x.v,pid_y.v, 0.0, pid_yaw.v);
