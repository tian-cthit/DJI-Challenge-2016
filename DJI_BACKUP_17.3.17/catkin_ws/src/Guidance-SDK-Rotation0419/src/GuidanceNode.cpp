...
ros::Publisher depth_image_pub[5];
ros::Publisher left_image_pub[5];
ros::Publisher right_image_pub[5];

char depth_pub_topic[5][100],left_pub_topic[5][100],right_pub_topic[5][100];
char camera_id[100];
...
/*
int crimminsImage(CvMat* image,int width,int height, int iterations)
{
	CvMat* image2 = image;
...
int my_callback(int data_type, int data_len, char *content)
{
	g_lock.enter();
        int k=1;  //迭代次数
	//sprintf(camera_id,"guidance_%d",camera_flag);
	/* image data */
	...
	{        
		...
		if ( data->m_greyscale_image_left[CAMERA_ID] ){
			...
			left_8.header.frame_id  = camera_id;
			left_8.header.stamp	= ros::Time::now();
			left_8.encoding		= sensor_msgs::image_encodings::MONO8;
			left_image_pub[camera_flag].publish(left_8.toImageMsg());
		}
		if ( data->m_greyscale_image_right[CAMERA_ID] ){
			...
			right_8.header.frame_id  = camera_id;
			right_8.header.stamp	 = ros::Time::now();
			right_8.encoding  	 = sensor_msgs::image_encodings::MONO8;
			right_image_pub[camera_flag].publish(right_8.toImageMsg());
		}
		...
		if ( data->m_depth_image[CAMERA_ID] ){
			...
			depth_16.header.frame_id  = camera_id;
			depth_16.header.stamp	  = ros::Time::now();
			depth_16.encoding	  = sensor_msgs::image_encodings::MONO16;
			depth_image_pub[camera_flag].publish(depth_16.toImageMsg());
		}
		//printf("CAMERA_ID = %d\n",CAMERA_ID);
		/*if(camera_flag < 1){
			camera_flag++;
	    		switch(camera_flag){
				case 1: CAMERA_ID = e_vbus2;
					break;
				case 2: CAMERA_ID = e_vbus3;
					break;
				case 3: CAMERA_ID = e_vbus4;
					break;
				case 4: CAMERA_ID = e_vbus5;
					break;
				default: CAMERA_ID = e_vbus1;
					break;
	    		}
	    	}
	    	else{
	...
	//	printf( "obstacle distance:" );
		...
		for ( int i = 1; i < /*CAMERA_PAIR_NUM*/2; ++i )
		{
	...
	if ( e_ultrasonic == data_type && NULL != content ){
		...
		for ( int d = 1; d < /*CAMERA_PAIR_NUM*/2; ++d )
		{
...
int main(int argc, char** argv)
{
	...
	for(int i=0;i<5;i++)
	{
		sprintf(depth_pub_topic[i],"/guidance/depth_image_%d",i);
		sprintf(left_pub_topic[i],"/guidance/left_image_%d",i);
		sprintf(right_pub_topic[i],"/guidance/right_image_%d",i);

		depth_image_pub[i] = my_node.advertise<sensor_msgs::Image>(depth_pub_topic[i],1);
		left_image_pub[i] = my_node.advertise<sensor_msgs::Image>(left_pub_topic[i],1);
		right_image_pub[i] = my_node.advertise<sensor_msgs::Image>(right_pub_topic[i],1);
	}
